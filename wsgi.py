...
class ControllerBase_gevent(object):
    def __init__(self,data,**config):
        self.data = data
        for name, value in config.items():
            setattr(self, name, value)

class WSGIApplication_gevent(object):
    def __init__(self, **config):
        self.config = config
        self.port = 8088
        self.controller = None
        super(WSGIApplication_gevent, self).__init__()
    def register(self,controller,data):
        self.controller = controller(data)
    

class WSGIServer_gevent(pywsgi.WSGIServer):
    def __init__(self, application, **config):
        super(WSGIServer_gevent, self).__init__(
            ("", application.port), application.controller.websocket_app,handler_class=WebSocketHandler,**config)
...
def start_service(app_mgr):
    for instance in app_mgr.contexts.values():
        ...
        if instance.__class__ == WSGIApplication_gevent:
            return WSGIServer_gevent(instance)
